...
def __null_to_nan(df: pl.DataFrame, ctrl: list[str], test: list[str]) -> pl.DataFrame:
    df = df.with_columns(
        [
            pl.col(ctrl).map_elements(lambda x: x if x else np.nan),
            pl.col(test).map_elements(lambda x: x if x else np.nan),
        ]
    )
...
def _add_ttest(df: pl.DataFrame, ctrl: list[str], test: list[str]) -> pl.DataFrame:
    ...
    ttest = (
        ...
        .map_rows(
            lambda x: sp.stats.ttest_ind(
                drop_nans(x[0]), drop_nans(x[1]), alternative=x[2], equal_var=False
            ),
        )
...
def _add_half_trpytic(df: pl.DataFrame) -> pl.DataFrame:
    df = df.with_columns(
        ...
        pl.col("Cleavage Type").is_in(["FULL_TRP", "N_SEMI", "C_SEMI"])
    ).with_columns(
...
def _add_cut_sites(df: pl.DataFrame) -> pl.DataFrame:
    df = df.with_columns(
        pl.when(pl.col("Cleavage Type") == "C_SEMI")
        .then(pl.format("{}{}", pl.col("Next AA"), pl.col("End") + 1))
        .when(pl.col("Cleavage Type") == "N_SEMI")
        .then(pl.format("{}{}", pl.col("Start AA"), pl.col("Start")))
        .when((pl.col("Cleavage Type") == "FULL_TRP") & (pl.col("Prev AA") != "-"))
        .then(
            pl.format(
                "{}{}-{}{}",
                pl.col("Prev AA"),
        ...
        pl.format("{}_{}", pl.col("Protein ID"), pl.col("Cut Site")).alias(
            "Cut Site ID"
        )
    )
...
def _log10(df: pl.DataFrame, col: str) -> pl.DataFrame:
    df = df.with_columns(-pl.col(col).log10().alias(f"-Log10 {col}"))
    return df
